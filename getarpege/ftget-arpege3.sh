...
############################################################################################
#CLEAN
echo '1.' Download ICMSHARP data for ${JJ}/${MM}/${AA} > log3.out
############################################################################################
#                                    Write new get file                                    #
############################################################################################
car=T0000P
...
echo 'user chikhiw Dilaw#31' > get_icmsharp3
chmod 700 get_icmsharp3
echo 'cd /home/m/mxpt/mxpt001/vortex/arpege/4dvarfr/OPER/'${AA}/${MM}/${JJ}'/'${car}'/forecast' >> get_icmsharp3
for iech in {31..40..01}  
do
echo get historic.arpege.tl1798-c22+00${iech}:00.fa >> get_icmsharp3
done
echo 'quit' >> get_icmsharp3
############################################################################################
#                              Get Arpege Files from HENDRIX                               #
############################################################################################
echo '2. Connecting to HENDRIX ....' >> log3.out
ftp -n hendrix < get_icmsharp3
sleep 5
for iech in  {31..40..01} 
do
historic.arpege.tl1798-c22+00${iech}:00.fa  $INPARP/ICMSHARPE_${AA}${MM}${DD}00${iech}
done
mv log3.out ${HOME}/FOG/getarpege/.
sleep 2
#rm -f get_icmsharp3
exit
